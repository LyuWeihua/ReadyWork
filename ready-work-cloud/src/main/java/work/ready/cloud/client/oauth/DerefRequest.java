...
import work.ready.cloud.client.oauth.config.DerefConfig;
...
public class DerefRequest {
    ...
    public DerefRequest(String token) {
        DerefConfig derefConfig = null;
        derefConfig = ReadyCloud.getConfig().getHttpClient().getOauth().getDeref();
        if (derefConfig != null) {
            setServerUrl(derefConfig.getServerUrl());
            setServiceId(derefConfig.getServiceId());
            setUri(derefConfig.getUri() + "/" + token);
            setClientId(derefConfig.getClientId());
            setClientSecret(derefConfig.getClientSecret());
        }
    }
