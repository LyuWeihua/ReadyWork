...
import work.ready.core.tools.define.CheckedBiFunction;
...
import static work.ready.cloud.jdbc.olap.proto.Protocol.SQL_QUERY_REST_ENDPOINT;
import static work.ready.core.tools.HttpUtil.appendSegmentToPath;
...
public class JreHttpUrlConnection implements Closeable {
    ...
    public static <R> R http(String path, String query, ConnectionConfiguration cfg, Function<JreHttpUrlConnection, R> handler) {
        final URI uriPath = appendSegmentToPath(cfg.baseUri(), path);  
        final String uriQuery = query == null ? uriPath.getQuery() : query; 
        final URL url;
        try {
    ...
    private void setupBasicAuth(ConnectionConfiguration cfg) {
        if (StrUtil.notBlank(cfg.authUser())) {
            String basicValue = cfg.authUser() + ":" + cfg.authPass();
            String encoded = StrUtil.asUTFString(Base64.getEncoder().encode(StrUtil.toUTF(basicValue)));
