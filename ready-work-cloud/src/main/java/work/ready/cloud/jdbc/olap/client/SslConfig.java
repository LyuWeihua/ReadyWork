...
public class SslConfig {
    ...
    public SslConfig(Properties settings, URI baseURI) {
        ...
        if (!isSSLPropertyPresent && !isSchemaPresent) {
            enabled = Boolean.parseBoolean(SSL_DEFAULT);
        } else {
            if (isSSLPropertyPresent && isHttpsScheme && !Boolean.parseBoolean(settings.getProperty(SSL))) {
                throw new ClientException("Cannot enable SSL: HTTPS protocol being used in the URL and SSL disabled in properties");
            }
            enabled = isHttpsScheme || Boolean.parseBoolean(settings.getProperty(SSL, SSL_DEFAULT));
        }
    ...
    private KeyManager[] loadKeyManagers() throws GeneralSecurityException, IOException {
        if (StrUtil.isBlank(keystoreLocation)) {
            return null;
        }

        char[] pass = (StrUtil.notBlank(keystorePass) ? keystorePass.trim().toCharArray() : null);
    ...
    private TrustManager[] loadTrustManagers() throws GeneralSecurityException, IOException {
        ...
        if (StrUtil.notBlank(truststoreLocation)) {
            char[] pass = (StrUtil.notBlank(truststorePass) ? truststorePass.trim().toCharArray() : null);
