...
public class MultiPartParserDefinition implements FormParserFactory.ParserDefinition<MultiPartParserDefinition> {
    ...
    private UploadPathProvider pathProvider;
    ...
    public void setDirectlySaveUploadFile(boolean directlySaveUploadFile) {
    ...
    public UploadPathProvider getPathProvider() {
        return pathProvider;
    }

    public void setPathProvider(UploadPathProvider pathProvider) {
    ...
    private final class MultiPartUploadHandler implements FormDataParser, MultipartParser.PartHandler {
        ...
        public void parse(final HttpHandler handler) throws Exception {
            ...
            if (executor == null) {
                exchange.dispatch(new MultiPartUploadHandler.NonBlockingParseTask(exchange.getConnection().getWorker(), requestChannel));
            } else {
                exchange.dispatch(executor, new MultiPartUploadHandler.NonBlockingParseTask(executor, requestChannel));
            }
        }
            ...
            if (disposition != null) {
                if (disposition.startsWith("form-data")) {
                    ...
                    if (fileName != null && fileSizeThreshold == 0) {
                        try {
                            if(directlySaveUploadFile){
                                if(pathProvider != null){
                                    file = Paths.get(getFileName(new File(pathProvider.getPath(currentName, fileName, uploadPath).toString())).getPath());
                                } else {
                                    file = Paths.get(getFileName(new File(uploadPath.toString() + File.separator + fileName)).getPath());
                                }
                            }else{
                                if (tempFileLocation != null) {
                                    file = Files.createTempFile(tempFileLocation, "ready", "work");
                                } else {
                                    file = Files.createTempFile("ready", "work");
                                }
                            }
        ...
        public void close() throws IOException {
            ...
            if(directlySaveUploadFile) return;
                            ...
                            if(c == 0) {
                                requestChannel.getReadSetter().set(new ChannelListener<StreamSourceChannel>() {
                                    ...
                                    public void handleEvent(StreamSourceChannel channel) {
                                        ...
                                        executor.execute(MultiPartUploadHandler.NonBlockingParseTask.this);
                                    }
                                });
