...
public class TimestampGetter extends ParamGetter<Timestamp> {
	...
	@Override
	public java.sql.Timestamp get(Action action, Controller c) {
	...
	protected java.sql.Timestamp to(String v) {
		...
		try {
			...
			throw new ApiException(new Status(PARAM_PARSE_EXCEPTION, v, java.sql.Timestamp.class));
		}
	}
