...
import work.ready.core.tools.ObjectUtil;
...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
	...
	public ConcurrentReferenceHashMap(int initialCapacity, float loadFactor, int concurrencyLevel,
		...
		Assert.that(initialCapacity >= 0).isTrue("Initial capacity must not be negative")
		.and(loadFactor > 0f).isTrue("Load factor must be positive")
		.and(concurrencyLevel > 0).isTrue("Concurrency level must be positive")
		.and(referenceType).notNull("Reference type must not be null");
	...
	private class EntryIterator implements Iterator<Map.Entry<K, V>> {
		...
		@Override
		public void remove() {
			Assert.that(this.last != null).isTrue("No element to remove");
