...
public class DataDictionaryGenerator {
	private static final Log logger = LogFactory.getLog(DataDictionaryGenerator.class);
	...
	public void setDataDictionaryOutputDir(String dataDictionaryOutputDir) {
		if (StrUtil.notBlank(dataDictionaryOutputDir)) {
			this.dataDictionaryOutputDir = dataDictionaryOutputDir;
		}
	}
	...
	public void setDataDictionaryFileName(String dataDictionaryFileName) {
		if (StrUtil.notBlank(dataDictionaryFileName)) {
	...
	protected void generateTable(TableMeta tableMeta, StringBuilder ret) {
		...
		if (StrUtil.notBlank(tableMeta.remarks)) {
	...
	protected void rebuildColumnMetas(List<TableMeta> tableMetas) {
		...
		try {
			...
			if (conn != null) {
				try {conn.close();} catch (SQLException e) {logger.error(e,"connection close exception");}
			}
		}
	}
	...
	protected void writeToFile(String ret) {
		...
		try {
			...
			if (osw != null) {
				try {osw.close();} catch (IOException e) {logger.error(e, "OutputStreamWriter close exception");}
			}
		}
	}
}
